...
namespace OH {
  class ADCNaiveBattery : public OH::AbstractBattery {
   ...
   public:
    ADCNaiveBattery(const uint8_t pin, OH::BatteryConfig config, OH::IEventDispatcher* eventDispatcher, TaskConfig taskConfig = { "ADC Battery", 1024, BATTERY_TASK_PRIORITY, tskNO_AFFINITY })
    ...
    ADCNaiveBattery(const uint8_t pin, OH::BatteryConfig config, OH::IEventDispatcher* eventDispatcher, const BaseType_t coreId = tskNO_AFFINITY)
      : AbstractBattery(config, eventDispatcher, { "ADC Battery", 1024, BATTERY_TASK_PRIORITY, coreId }), pin(pin) {};
