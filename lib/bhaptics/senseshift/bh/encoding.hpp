...
class Decoder {
  public:
    ...
    template<size_t N>
    static void
      applyPlain(FloatBody* output, std::string& value, const std::array<OutputLayout, N>& layout, const Effect effect)
    {
    ...
    )
    {
        ...
        for (uint8_t i = 0; i < VEST_LAYOUT_SIZE; i++) {
            ...
            output->effect(target, position, static_cast<FloatBody::Plane::Value>(effectDataFromByte(result[i], 15)));
        }
    }
